...
export default class Books extends Component {
    ...
    componentDidMount() {
        const ref = firebase.database().ref('images/Books & Stationery');
    ...
    uidData =  (data) => {
      ...
      if(data.exists()) {
        ...
        for(var i=0; i<keys.length;i++) {     
          const ref = firebase.database().ref('images/Books & Stationery').child(keys[i]);
                            ...
                            return (
                                <TouchableOpacity style={styles.containerImg}
                                  onPress={() => this.props.navigation.navigate('ReplyBags',
                                   {uid: item.uid, name: item.name, title: item.title, comments: item.comments, url: item.url, category: "Books & Stationery"})}>
