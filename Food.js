...
export default class Food extends Component {
    ...
    componentDidMount() {
        const ref = firebase.database().ref('images/Food');
    ...
    uidData =  (data) => {
      ...
      if(data.exists()) {
        ...
        for(var i=0; i<keys.length;i++) {     
          const ref = firebase.database().ref('images/Food').child(keys[i]);
          ...
          style={styles.imgBackground}>   
              <Header
                backgroundColor= {'#4b0082'}
                  ...
                  centerComponent={{text: 'Food', style: {color: 'white', fontSize: 23,
                            ...
                            return (
                                <TouchableOpacity style={styles.containerImg}
                                  onPress={() => this.props.navigation.navigate('ReplyBags',
                                   {uid: item.uid, name: item.name, title: item.title, comments: item.comments, url: item.url, category: "Food"})}>
